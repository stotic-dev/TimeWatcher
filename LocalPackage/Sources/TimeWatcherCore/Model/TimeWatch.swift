...
public class TimeWatch {
    ...
    public init(publisher: AnyPublisher<Date, Never>? = nil,
    ...
    public func createTimerStatusPublisher() -> AnyPublisher<TimerStatus, Never> {

        return timerStatusPublisher.eraseToAnyPublisher()
    }

    /// timeWatchIntervalの間隔ごとに経過時間を通知するクロージャを設定する
    public func setTimerHandler(timeLapseHandler: @escaping (TimeInterval) -> Void) {
    ...
    /// 現在の経過時間総計を返す
    public func getCurrentTimeLapse() -> TimeInterval {
